...
首次部署使用 `POST /bootstrap`，令牌来自 `BOOTSTRAP_TOKEN`，成功后永久关闭。`POST /auth/login` 返回短时 access token；Web refresh token 是 `Secure`、`HttpOnly`、`SameSite=Lax` Cookie，原生容器应通过平台安全存储持有。每次 refresh 都轮换 token，旧链重放会撤销该设备会话链。
