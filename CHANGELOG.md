...
- 彻底移除「归档」机制：任务、目录、事件与流程只支持「永久删除」。删除事件时会级联移除其中的任务安排，任务本体保留在目录中；删除任务、目录与流程的行为与之前一致（均带确认，不可恢复）。
- 「安排任务」弹窗移除「新建任务并安排」页签，只保留从目录选择已有任务加入日程。
- 「快速添加」「捕获」「上下文」三个冗余入口删除：顶栏保留搜索按钮，今日页只保留任务列表与进度。

### 修复

- 修复任务行溢出菜单被后续卡片或输入框覆盖的层级问题：菜单抬升至内容层之上，打开菜单的行整体置顶。
- 修复日历等页面在小屏设备上横向溢出、需要左右滚动的问题：页面容器禁止横向溢出，宽内容自动收缩换行。
- 修复目录页进出子目录后整个页面滚动位置被重置的问题：滚动重置与路由重挂载改为只在切换顶级页面时发生。

### 使用注意

- 本次包含数据库迁移 `0010_remove_archive.sql`：历史归档数据会被转换为「已删除」状态，迁移后无法恢复归档，升级前请先按备份文档完成数据库备份。
- 「安排任务」不再支持新建任务，请先在目录中创建任务再加入日程。
...
- Linux：已提供
- Android：已提供
- Docker：`ghcr.io/igngserver/taskdock:2.0.0-rc.26`

## 2.0.0-rc.25 · 2026-09-26

### 新增

- 中枢登录限流上限改为可配置项 `AUTH_LOGIN_RATE_LIMIT`：默认仍是每个「客户端地址 + 用户名」15 分钟 10 次，大量用户共享同一出口地址（NAT、企业代理）的部署可以适当放宽。这是安全边界上的旋钮，因此生产环境取值大于 1000 时中枢直接拒绝启动。

### 质量门禁

- 修复真实链路 e2e 门禁此前从未真正验证生产构建的问题：CI 只设置 `E2E_REAL=1` 而没有 `E2E_PWA=1`，`e2e-real-server.ts` 因此用 vite 开发服务器提供页面，依赖生产产物与 service worker 的 `mobile-pwa`、`real-smoke` 不可能通过；同时把靠 `page.route` 伪造接口故障的用例（`calendar-error-state`、`tree-error-state`、`reload-error-states`）限定回 mocked 门禁，两个门禁各测各的。此前 master 长期红灯正是这两处叠加：先卡在过期用例上，修好后才暴露真实链路门禁的问题。
- 修复移动端 PWA 用例点击已不存在的 `/tasks` 抽屉项导致每次 90 秒超时；`/tasks` 现在是指向 `/tree` 的重定向别名。
- 无障碍审计改为在禁用动画的状态下测量颜色，消除 webkit 上的偶发误报：被审计内容会在等待返回之后才挂载，任何"等动画结束"的策略都覆盖不到这种情况，实测半透明卡片的对比度只有 3.7:1。
- CI 在失败时保留 Playwright 的 `error-context.md` 页面快照、trace 与报告，保留 7 天。
